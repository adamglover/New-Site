...
class PerchFieldType_vimeo extends PerchAPI_FieldType
{
    ...
    public function render_inputs($details=array())
    {
        ...
        if (isset($json['vimeoID'])) {
            ...
            $w = 320;
            $h = $w*$ratio;
            $s.= '<div class="preview"><iframe src="http://player.vimeo.com/video/'.$json['vimeoID'].'?portrait=0&amp;badge=0&amp;color=ffffff" width="'.$w.'" height="'.$h.'" frameborder="0"></iframe></div>';
        }
            ...
            if ($this->Tag->output() && $this->Tag->output()!='path') {
                switch($this->Tag->output()) {        
                    ...
                    case 'embed':
                        ...
                        return '<iframe src="http://player.vimeo.com/video/'.$item['vimeoID'].'" width="'.$w.'" height="'.$h.'" frameborder="0" webkitAllowFullScreen mozallowfullscreen allowFullScreen></iframe>';
